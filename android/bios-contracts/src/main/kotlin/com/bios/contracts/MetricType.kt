...
enum class MetricType(
    ...
    D_DIMER("d_dimer", MetricUnit.NG_PER_ML, MetricDomain.BIOMARKER, allowsManualEntry = true), REVERSE_T3("reverse_t3", MetricUnit.NG_PER_DL, MetricDomain.BIOMARKER, allowsManualEntry = true), OMEGA_3_INDEX("omega_3_index", MetricUnit.PERCENT, MetricDomain.BIOMARKER, allowsManualEntry = true), LEPTIN("leptin", MetricUnit.NG_PER_ML, MetricDomain.BIOMARKER, allowsManualEntry = true), ADIPONECTIN("adiponectin", MetricUnit.MG_PER_L, MetricDomain.BIOMARKER, allowsManualEntry = true), // Wave 6 expansion (#339) — D-dimer ng/mL is FEU (Fibrinogen Equivalent Units, dominant US/EU convention); reverse T3, leptin, adiponectin descriptive-only until population-specific bands settle (sex/age/BMI dependence per issue). ALP + Lp(a) already shipped; #339 completes their bands + pattern corroborator wiring.
