...
/**
 * Phase 1-4 baseline-deviation patterns: the 11 founding ConditionPatterns
 * that fire when an owner's signals drift from their personal baseline.
 *
 * Extracted from [ConditionPatterns] to keep that file under the 500-line
 * cap and to give new alerts PRs a dedicated landing zone — adding a
 * baseline-deviation pattern means adding to this file, not editing the
 * aggregator. Specialised pattern groups (sepsis, afib rhythm, pregnancy,
 * heart-failure, tropical disease, headache, etc.) already live in their
 * own files; this one is the catch-all for the original general-purpose
 * detection set.
 */
internal object BaselineDeviationPatterns {

    val all: List<ConditionPattern> by lazy {
        listOf(
    ...
    val sleepDisruption = ConditionPattern(
        ...
        // #189 + #186 gate: fragmentation physiologic in T3/postpartum; deconditioned baseline in FRAILTY_FLAG false-fires.
    ...
    val cardiorespiratoryDeconditioning = ConditionPattern(
        ...
        signalRules = listOf(
            ...
            // Direct VO2 max signal — the clinical gold standard.
    ...
    val recoveryDeficit = ConditionPattern(
        ...
        // #189 + #186 gate: HRV↓+sleep↓+RHR↑ is normal pregnancy/postpartum and the FRAILTY_FLAG baseline.
        excludedStates = PhysiologyState.PREGNANCY + setOf(PhysiologyState.POSTPARTUM, PhysiologyState.FRAILTY_FLAG),
    )
     ...
     *  Reports data only — never diagnoses. */
     ...
     * has enabled BBT tracking, and the BaselineEngine computes a BBT baseline
     * from the same source — its statistical summary lives in the main DB, no
     * raw values escape. The BBT signal anchors the pattern when the owner is
     * tracking; HRV + sleep corroborators still carry the pattern at
     * minActiveSignals = 2 when no reproductive DB is available.
     */
